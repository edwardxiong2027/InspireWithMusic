...
type CmsContextValue={content:CmsMap;preview:boolean};
const CmsContext=createContext<CmsContextValue>({content:{},preview:false});
function useCms(){const {content,preview}=useContext(CmsContext);return {text:(key:string,fallback:string)=>content[key]||fallback,number:(key:string,fallback:number)=>Number(content[key]||fallback),json:<T,>(key:string,fallback:T):T=>{try{return content[key]?JSON.parse(content[key]) as T:fallback}catch{return fallback}},bind:(key:string)=>preview?{"data-cms-key":key}:{} as Record<string,string>}}
function CmsProvider({children}:{children:React.ReactNode}){const [content,setContent]=useState<CmsMap>({});const [preview,setPreview]=useState(false);useEffect(()=>{const active=new URLSearchParams(window.location.search).get("cmsPreview")==="1";setPreview(active);getPublicContent().then(remote=>setContent(current=>({...remote,...current}))).catch(()=>{});if(!active)return;document.body.classList.add("cms-preview-active");const receive=(event:MessageEvent)=>{if(event.origin===window.location.origin&&event.data?.type==="IWM_CMS_DRAFT")setContent(current=>({...current,...event.data.content}))};const select=(event:MouseEvent)=>{const element=(event.target as Element|null)?.closest<HTMLElement>("[data-cms-key]");if(element){event.preventDefault();event.stopPropagation();window.parent.postMessage({type:"IWM_CMS_SELECT",key:element.dataset.cmsKey},window.location.origin)}else if((event.target as Element|null)?.closest("a,button")){event.preventDefault();event.stopPropagation()}};window.addEventListener("message",receive);document.addEventListener("click",select,true);window.parent.postMessage({type:"IWM_CMS_READY"},window.location.origin);return()=>{document.body.classList.remove("cms-preview-active");window.removeEventListener("message",receive);document.removeEventListener("click",select,true)}},[]);return <CmsContext.Provider value={{content,preview}}>{children}</CmsContext.Provider>}
...
function Footer() {
  ...
  return <footer>
    <div className="footer-top">
      ...
      <div className="footer-links"><span>CONNECT</span><a {...bind("footer.contact_email")} href={`mailto:${text("footer.contact_email","hello@inspirewithmusic.org")}`}>Email us</a><a {...bind("footer.instagram_url")} href={text("footer.instagram_url","#")}>Instagram</a><a {...bind("footer.youtube_url")} href={text("footer.youtube_url","#")}>YouTube</a><button onClick={() => go("login")}>Member portal</button></div>
...
function Home() {
  const {text,number,json,bind}=useCms();
  ...
  return <>
    <section className="hero">
      <img {...bind("home.hero_image")} src={text("home.hero_image",officialImages.hero)} alt="Youth string orchestra performing on stage" />
      <div className="hero-shade"/><div className="hero-score" aria-hidden="true"><i/><i/><i/><i/><i/></div>
      <div className="hero-content"><p className="hero-kicker" {...bind("home.hero_eyebrow")}>{text("home.hero_eyebrow","YOUTH-LED · MUSIC-DRIVEN · COMMUNITY-FOCUSED")}</p><h1 {...bind("home.hero_title")}>{text("home.hero_title","Music in Action. Youth in Service.")}</h1><p className="hero-copy" {...bind("home.hero_text")}>{text("home.hero_text","Empowering young musicians to share their talents, serve their communities, and make a difference through music.")}</p><div className="hero-actions"><button className="button coral" onClick={() => go("join")}>Join as a volunteer <Arrow /></button><button className="button glass" onClick={() => go("impact")}>Explore our impact <Arrow /></button></div></div>
    ...
    <section className="story-section">
      <div className="story-image"><img {...bind("home.story_image")} src={text("home.story_image",officialImages.travel)} alt="Youth musicians on a service journey"/><span className="vertical-label">OUR STORY · SINCE 2019</span></div>
      <div className="story-copy"><p className="eyebrow"><span>04</span> OUR STORY</p><h2 {...bind("home.story_title")}>{text("home.story_title","Every note has led us here.")}</h2><p {...bind("home.story_body")}>{text("home.story_body","From serving our local communities to connecting with audiences around the world, our journey has been shaped by music, service, and a growing community of young musicians.")}</p><div className="timeline" {...bind("home.story_timeline_json")}>{storyTimeline.map((item,index)=><div className={index===storyTimeline.length-1?"now":""} key={`${item.title}-${index}`}><b>{item.title}</b><span>{item.label}</span></div>)}</div><button className="button ink" onClick={() => go("about")}>Follow our journey <Arrow /></button></div>
    </section>

    <section className="impact-section section-pad">
      <div className="impact-lead"><p className="eyebrow light"><span>05</span> IMPACT IN NUMBERS</p><h2><span {...bind("home.impact_title")}>{text("home.impact_title","A growing community.")}</span><br/><em {...bind("home.impact_accent")}>{text("home.impact_accent","A lasting echo.")}</em></h2><p {...bind("home.impact_body")}>{text("home.impact_body","Behind every number is a story of young musicians sharing their time, talent, and love of music with the community.")}</p></div>
    ...
    <section className="join-band"><div><p className="eyebrow light"><span>08</span> JOIN THE MOVEMENT</p><h2 {...bind("home.join_title")}>{text("home.join_title","Your talent can change the tempo.")}</h2></div><div><p>Turn your love of music into meaningful service. There’s a place for your sound here.</p><button className="button coral" onClick={() => go("join")}>Find your place <Arrow /></button></div><span className="join-note" aria-hidden="true">♫</span></section>
  </>;
}

function InnerHero({ eyebrow, title, italic, text, image = officialImages.hero, titleKey, textKey, imageKey }: { eyebrow: string; title: string; italic?: string; text: string; image?: string; titleKey?:string; textKey?:string; imageKey?:string }) {
  const {bind}=useCms();return <section className="inner-hero"><img {...(imageKey?bind(imageKey):{})} src={image} alt="Ivy Chamber Strings youth musicians"/><div/><p className="eyebrow light"><span>♪</span>{eyebrow}</p><h1 {...(titleKey?bind(titleKey):{})}>{title}<br/><em>{italic}</em></h1><p className="inner-dek" {...(textKey?bind(textKey):{})}>{text}</p></section>;
}

function About() { const {text,bind}=useCms(); return <>
  <InnerHero eyebrow="ABOUT US" title={text("about.hero_title","A love of music. A call to serve.")} text={text("about.hero_text","We believe young people are not only future leaders—they are powerful leaders right now.")} image={text("about.hero_image",officialImages.orchestra)} titleKey="about.hero_title" textKey="about.hero_text" imageKey="about.hero_image"/>
  <section className="split-copy section-pad"><p className="eyebrow"><span>01</span> OUR MISSION</p><h2 {...bind("about.mission")}>{text("about.mission","We make space for young musicians to lead with heart.")}</h2><div><p {...bind("about.body")}>{text("about.body","Ivy Chamber Strings is a 501(c)(3) nonprofit youth orchestra and chamber music program serving students ages 6 to 17 in Orange County. We connect musical growth with purposeful service.")}</p></div></section>
...
function Volunteers() { const {text}=useCms(); return <>
  <InnerHero eyebrow="OUR VOLUNTEERS" title={text("volunteers.hero_title","Many instruments. One shared purpose.")} text={text("volunteers.hero_text","Meet the young people turning rehearsal-room discipline into real-world compassion.")} image={text("volunteers.hero_image",officialImages.community)} titleKey="volunteers.hero_title" textKey="volunteers.hero_text" imageKey="volunteers.hero_image"/>
...
function Programs() { const {text,json}=useCms(); const items=json("programs.items_json",defaultPrograms);const slugFromHash=()=>typeof window==="undefined"?"":window.location.hash.split("/")[2]||"";const [selected,setSelected]=useState(slugFromHash);useEffect(()=>{const sync=()=>setSelected(slugFromHash());window.addEventListener("hashchange",sync);return()=>window.removeEventListener("hashchange",sync)},[]);const program=items.find(item=>item.slug===selected);if(program)return <>
  ...
  return <><InnerHero eyebrow="OUR PROGRAMS" title={text("programs.hero_title","Music that moves into the world.")} text={text("programs.hero_text","Four youth-led programs. Countless ways to connect.")} image={text("programs.hero_image",officialImages.stage)} titleKey="programs.hero_title" textKey="programs.hero_text" imageKey="programs.hero_image"/><section className="program-detail section-pad" data-cms-key="programs.items_json">{items.map(p=><article key={p.title} role="link" tabIndex={0} onClick={()=>{goProgram(p.slug);setSelected(p.slug)}} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){goProgram(p.slug);setSelected(p.slug)}}}><div className="program-detail-image"><img src={p.image} alt={p.title}/><span>{p.number}</span></div><div><p className="eyebrow">PROGRAM {p.number}</p><h2>{p.title}</h2><p>{p.text}</p><button className="text-link">Explore this program <Arrow/></button></div></article>)}</section><JoinMini /></>; }

function Stories() { const {text}=useCms(); return <>
  <InnerHero eyebrow="STORIES IN ACTION" title={text("stories.hero_title","The moments between the notes.")} text={text("stories.hero_text","Student-written stories, photos, and reflections from a community in motion.")} image={text("stories.hero_image",officialImages.travel)} titleKey="stories.hero_title" textKey="stories.hero_text" imageKey="stories.hero_image"/>
...
function Impact() { const {text,number,bind}=useCms(); return <>
  <InnerHero eyebrow="OUR IMPACT" title={text("impact.hero_title","What service sounds like.")} text={text("impact.hero_text","Hours are part of the story. People, confidence, connection, and access complete it.")} image={text("impact.hero_image",officialImages.hero)} titleKey="impact.hero_title" textKey="impact.hero_text" imageKey="impact.hero_image"/>
  <section className="impact-page section-pad"><div {...bind("impact.section_title")}><SectionHeading eyebrow="LIVE IMPACT" title={text("impact.section_title","A growing community. A lasting echo.")}/></div><p className="impact-intro" {...bind("impact.section_body")}>{text("impact.section_body","Behind every number is a story of young musicians sharing their time, talent, and love of music with the community.")}</p><div className="impact-big">{[[`${number("impact.volunteers",120).toLocaleString()}+`,"Youth volunteers","impact.volunteers"],[`${number("impact.hours",3800).toLocaleString()}+`,"Service hours","impact.hours"],[`${number("impact.events",86).toLocaleString()}+`,"Community events","impact.events"],[`${number("impact.people",12000).toLocaleString()}+`,"People reached","impact.people"],[`${text("impact.music_shared","XXX")}+`,"Music shared","impact.music_shared"]].map(x=><div key={x[1]} {...bind(x[2])}><b>{x[0]}</b><span>{x[1]}</span><i/></div>)}</div></section>
...
function Join() { const {text}=useCms(); const [sent,setSent]=useState(""); async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const form=new FormData(e.currentTarget);try{await submitContact(Object.fromEntries(form));setSent("Thank you—your message has been sent to the webmaster.");e.currentTarget.reset()}catch{setSent("Please check your information and try again.")}} return <>
  <InnerHero eyebrow="JOIN THE MOVEMENT" title={text("join.hero_title","Bring what you love. Leave an echo.")} text={text("join.hero_text","Volunteer, partner, or bring a program to your community. Your next step starts here.")} image={text("join.hero_image",officialImages.orchestra)} titleKey="join.hero_title" textKey="join.hero_text" imageKey="join.hero_image"/>
...
function Donate() { const {text,bind}=useCms(); return <>
  <section className="donate-hero"><div className="donate-note" aria-hidden="true">♪</div><p className="eyebrow light">SUPPORT OUR MISSION</p><h1 {...bind("donate.hero_title")}>{text("donate.hero_title","Help the next generation be heard.")}</h1><p {...bind("donate.hero_text")}>{text("donate.hero_text","Your support helps us expand youth-led music programs, community performances, educational opportunities, and access to music.")}</p></section>
  <section className="give-section section-pad"><SectionHeading eyebrow="WAYS TO GIVE" title="Choose your way to make a difference."/><div className="give-grid"><article><span>Z</span><p>DIRECT GIVING</p><h2>Zelle</h2><p>Make a direct, secure gift using the organization’s verified Zelle details.</p><div className="placeholder-data" {...bind("donate.zelle")}>{text("donate.zelle","Add verified Zelle information")}</div></article><article className="paypal"><span>P</span><p>ONLINE GIVING</p><h2>PayPal</h2><p>Support our work with a one-time or recurring gift through PayPal.</p><a className="button ink" {...bind("donate.paypal_url")} href={text("donate.paypal_url","#")}>Donate with PayPal <Arrow diagonal/></a></article></div></section>
  <section className="materials"><img {...bind("donate.materials_image")} src={text("donate.materials_image",officialImages.community)} alt="Donating music education materials"/><div><p className="eyebrow light">DONATE MUSIC MATERIALS</p><h2>Give your music<br/><em>a second movement.</em></h2><p>Support our Music Exchange program by donating sheet music, music books, instruments, and other educational materials.</p><a className="button coral" {...bind("donate.materials_url")} href={text("donate.materials_url","#")}>Donate music materials <Arrow/></a></div></section>
