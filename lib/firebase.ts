...
function withId<T>(id: string, data: DocumentData) {
  return { id, ...data } as T;
}
...
function friendlyError(error: unknown) {
  ...
  const messages: Record<string, string> = {
    ...
    "auth/popup-blocked":
      "Your browser blocked Google sign-in. Please allow pop-ups and try again.",
...
async function ensureProfile(
  user: User,
  initial?: { name?: string; instrument?: string },
) {
  ...
  const load = (async () => {
    ...
    if (isBootstrap && !user.emailVerified)
      throw new Error(
        "Verify the email sent to inspirewithmusic.org@gmail.com, then sign in again to activate the webmaster account.",
      );
    const profile = {
      email: user.email ?? "",
...
async function ensureFirebaseSeed() {
  ...
  for (const entry of defaultContentEntries)
    if (!existing.has(entry.key))
      batch.set(doc(firestore, "content", entry.key), {
        ...entry,
        updated_at: now(),
      });
  if (!settingsSnapshot.exists())
    batch.set(doc(firestore, "settings", "site"), {
...
export async function getPublicContent() {
  try {
    ...
    return {
      ...defaultContentMap,
      ...Object.fromEntries(
        snapshot.docs.map((item) => [item.id, String(item.data().value ?? "")]),
      ),
    };
  } catch {
...
async function contentResponse() {
  ...
  const entries = defaultContentEntries.map((entry) =>
    remote.has(entry.key)
      ? { ...entry, ...remote.get(entry.key), key: entry.key }
      : entry,
  );
  return {
    entries,
...
async function eventResponse() {
  ...
  const user = await authUser();
  const session = user ? await ensureProfile(user) : null;
  ...
  const events = await Promise.all(
    snapshot.docs.map(async (item) => {
      ...
      const signupOpen =
        data.status === "open" &&
        currentTime < new Date(data.starts_at).getTime();
      const eligible =
        audience !== "official_members" ||
        session?.membership_status === "official_member";
      const normalized = {
        ...data,
        audience,
        signup_open: signupOpen,
        can_signup: signupOpen && eligible,
        signup_block_reason: !signupOpen
          ? "Signup closed"
          : !eligible
      ...
      const signup = await getDoc(
        doc(firestore, "events", item.id, "signups", user.uid),
      );
      return { ...normalized, is_signed_up: signup.exists() };
    }),
  );
  return {
...
async function hoursResponse() {
  ...
  return {
    hours: sortNewest(
      snapshot.docs.map((item) =>
        withId<ServiceHourRecord & { created_at: string }>(
          item.id,
          item.data(),
        ),
      ),
...
export async function firebaseApi<T>(
  url: string,
  options?: FirebaseApiOptions,
): Promise<T> {
  ...
  try {
    ...
    if (url === "/api/content" && method === "GET")
      return (await contentResponse()) as T;
    if (url === "/api/content" && method === "PUT") {
      await requireSession(["webmaster"]);
      const batch = writeBatch(firestore);
      for (const change of input.entries as Array<{
        key: string;
        value: string;
      }>) {
        const definition = defaultContentEntries.find(
          (entry) => entry.key === change.key,
        );
        if (!definition) continue;
        batch.set(doc(firestore, "content", change.key), {
          ...definition,
    ...
    if (url === "/api/events" && method === "GET")
      return (await eventResponse()) as T;
    if (url === "/api/events" && method === "POST") {
      const session = await requireSession(["volunteer_admin", "webmaster"]);
      if (
        new Date(String(input.endsAt)).getTime() <=
        new Date(String(input.startsAt)).getTime()
      )
        throw new Error("Event end time must be after its start time.");
      if (
        Number(input.capacity ?? 0) < 1 ||
        Number(input.serviceMinutes ?? 0) < 1
      )
        throw new Error("Capacity and service minutes must be positive.");
      const created = await addDoc(collection(firestore, "events"), {
        title: String(input.title ?? ""),
        description: String(input.description ?? ""),
        location: String(input.location ?? ""),
        starts_at: String(input.startsAt ?? ""),
        ends_at: String(input.endsAt ?? ""),
        starts_at_timestamp: Timestamp.fromDate(
          new Date(String(input.startsAt)),
        ),
        ends_at_timestamp: Timestamp.fromDate(new Date(String(input.endsAt))),
        capacity: Number(input.capacity ?? 20),
        service_minutes: Number(input.serviceMinutes ?? 0),
        status: String(input.status ?? "open"),
        audience: String(input.audience ?? "all_users"),
        signup_count: 0,
    ...
    if (signupMatch) {
      ...
      await runTransaction(firestore, async (transaction) => {
        ...
        const count = Number(event.signup_count ?? 0);
        const started =
          Date.now() >= new Date(String(event.starts_at)).getTime();
        if (method === "POST") {
          if (signupSnap.exists()) return;
          if (event.status !== "open" || started)
          ...
          transaction.set(signupRef, {
            ...
            member_instrument: session.instrument ?? "",
            source: "member",
          ...
          if (event.status !== "open" || started)
            throw new Error(
              "Online cancellation is closed. Please contact an administrator.",
            );
          transaction.delete(signupRef);
          transaction.update(eventRef, {
            signup_count: Math.max(0, count - 1),
            updated_at: now(),
          });
        }
      });
    ...
    if (signupListMatch) {
      await requireSession(["volunteer_admin", "webmaster"]);
      const eventId = signupListMatch[1];
      if (method === "GET") {
        const [signupSnapshot, userSnapshot] = await Promise.all([
          getDocs(collection(firestore, "events", eventId, "signups")),
          getDocs(collection(firestore, "users")),
        ]);
        return {
          signups: signupSnapshot.docs
            .map((item) => withId<EventSignup>(item.id, item.data()))
      ...
      if (method === "POST") {
        ...
        if (!memberSnap.exists() || !eventSnap.exists())
        ...
        const signupRef = doc(firestore, "events", eventId, "signups", userId);
        await runTransaction(firestore, async (transaction) => {
          const [currentEvent, currentSignup] = await Promise.all([
            transaction.get(eventRef),
            transaction.get(signupRef),
          ]);
          if (currentSignup.exists()) return;
          if (!currentEvent.exists()) throw new Error("Event not found.");
          const event = currentEvent.data();
          const count = Number(event.signup_count ?? 0);
          if (count >= Number(event.capacity))
            throw new Error(
              "This event is full. Increase capacity before adding another volunteer.",
            );
          transaction.set(signupRef, {
            user_id: userId,
            member_name: String(member.name ?? "Member"),
            member_email: String(member.email ?? ""),
            member_instrument: String(member.instrument ?? ""),
            source: "admin",
            attendance_status: "pending",
            created_at: now(),
            updated_at: now(),
    ...
    const signupAdminMatch = url.match(
      /^\/api\/events\/([^/]+)\/signups\/([^/]+)$/,
    );
    if (signupAdminMatch) {
      const reviewer = await requireSession(["volunteer_admin", "webmaster"]);
      const [, eventId, userId] = signupAdminMatch;
      const eventRef = doc(firestore, "events", eventId);
      const signupRef = doc(firestore, "events", eventId, "signups", userId);
      const hourRef = doc(firestore, "service_hours", `${eventId}_${userId}`);
      if (method === "PATCH") {
        const attendance = String(input.attendance);
        if (!["pending", "attended", "absent"].includes(attendance))
          throw new Error("Choose a valid attendance status.");
        await runTransaction(firestore, async (transaction) => {
          const [eventSnap, signupSnap, hourSnap] = await Promise.all([
            transaction.get(eventRef),
            transaction.get(signupRef),
            transaction.get(hourRef),
          ]);
          if (!eventSnap.exists() || !signupSnap.exists())
            throw new Error("Event signup not found.");
          const event = eventSnap.data();
          if (Date.now() < new Date(String(event.ends_at)).getTime())
            throw new Error(
              "Attendance can only be confirmed after the event ends.",
            );
          const signup = signupSnap.data();
          transaction.update(signupRef, {
            attendance_status: attendance,
            attendance_confirmed_by: reviewer.id,
            attendance_confirmed_at: now(),
            updated_at: now(),
          });
          if (attendance === "attended") {
            const minutes = Number(event.service_minutes ?? 0);
            if (minutes <= 0)
              throw new Error(
                "Set positive service minutes on the event before awarding attendance.",
              );
            transaction.set(hourRef, {
              user_id: userId,
              event_id: eventId,
              activity: String(event.title ?? "Volunteer event"),
              service_date: String(event.starts_at ?? "").slice(0, 10),
              minutes,
              notes: `Attendance confirmed for ${String(event.title ?? "event")}`,
              status: "verified",
              member_name: String(signup.member_name ?? "Member"),
              member_email: String(signup.member_email ?? ""),
              source: "event_attendance",
              verified_by: reviewer.id,
              verified_at: now(),
              updated_at: now(),
              created_at: hourSnap.exists()
                ? String(hourSnap.data().created_at ?? now())
                : now(),
            });
          } else if (hourSnap.exists()) {
            transaction.update(hourRef, {
              status: "rejected",
              notes:
                attendance === "absent"
                  ? "Marked absent by administrator"
                  : "Attendance confirmation reset",
              verified_by: reviewer.id,
              verified_at: now(),
              updated_at: now(),
            });
          ...
          if (eventSnap.exists())
            transaction.update(eventRef, {
              signup_count: Math.max(
                0,
                Number(eventSnap.data().signup_count ?? 0) - 1,
              ),
              updated_at: now(),
            });
          if (hourSnap.exists())
            transaction.update(hourRef, {
              status: "rejected",
              notes: "Signup removed by administrator",
              verified_by: reviewer.id,
              verified_at: now(),
              updated_at: now(),
            });
    ...
    if (eventMatch) {
      const session = await requireSession(["volunteer_admin", "webmaster"]);
      const eventRef = doc(firestore, "events", eventMatch[1]);
      if (method === "PATCH") {
        ...
        const serviceMinutes =
          input.serviceMinutes !== undefined
            ? Number(input.serviceMinutes)
            : Number(current.service_minutes);
        if (new Date(endsAt).getTime() <= new Date(startsAt).getTime())
          throw new Error("Event end time must be after its start time.");
        if (capacity < Number(current.signup_count ?? 0))
          throw new Error(
            "Capacity cannot be lower than the current signup count.",
          );
        if (capacity < 1 || serviceMinutes < 1)
          throw new Error("Capacity and service minutes must be positive.");
        const change: Json = { updated_at: now() };
        if (input.title !== undefined) {
          change.title = String(input.title);
        }
        if (input.description !== undefined)
          change.description = String(input.description);
        if (input.location !== undefined)
          change.location = String(input.location);
        if (input.startsAt !== undefined) {
          change.starts_at = startsAt;
          change.starts_at_timestamp = Timestamp.fromDate(new Date(startsAt));
        }
        if (input.endsAt !== undefined) {
          change.ends_at = endsAt;
          change.ends_at_timestamp = Timestamp.fromDate(new Date(endsAt));
        }
        if (input.capacity !== undefined) change.capacity = capacity;
        if (input.serviceMinutes !== undefined)
          change.service_minutes = serviceMinutes;
        if (input.status !== undefined) change.status = String(input.status);
        if (input.audience !== undefined)
          change.audience = String(input.audience);
        await updateDoc(eventRef, change);
        return { ok: true } as T;
      }
      if (method === "DELETE") {
        if (session.role !== "webmaster")
          throw new Error("Only the Webmaster can delete events.");
        const signups = await getDocs(
          collection(firestore, "events", eventMatch[1], "signups"),
        );
        const batch = writeBatch(firestore);
        signups.docs.forEach((item) => batch.delete(item.ref));
    ...
    if (url === "/api/hours" && method === "POST") {
      ...
      const created = await addDoc(collection(firestore, "service_hours"), {
        ...
        service_date: String(input.serviceDate ?? ""),
    ...
    if (hourMatch && method === "PATCH") {
      const reviewer = await requireSession(["webmaster"]);
      await updateDoc(doc(firestore, "service_hours", hourMatch[1]), {
    ...
    if (url === "/api/profile" && method === "PATCH") {
      ...
      const updated = {
        ...session,
        name: String(input.name ?? session.name),
        phone: String(input.phone ?? ""),
        instrument: String(input.instrument ?? ""),
      };
      await updateDoc(doc(firestore, "users", session.id), {
        name: updated.name,
        phone: updated.phone,
        instrument: updated.instrument,
        updated_at: now(),
      });
      profileCache.set(session.id, updated);
      return { ok: true } as T;
    }
    if (url === "/api/users" && method === "GET") {
      await requireSession(["volunteer_admin", "webmaster"]);
      const [usersSnapshot, hoursSnapshot] = await Promise.all([
        getDocs(collection(firestore, "users")),
        getDocs(
          query(
            collection(firestore, "service_hours"),
      ...
      hoursSnapshot.docs.forEach((item) =>
        totals.set(
          String(item.data().user_id),
          (totals.get(String(item.data().user_id)) ?? 0) +
            Number(item.data().minutes ?? 0),
        ),
      );
      return {
        users: usersSnapshot.docs.map((item) => {
    ...
    if (
      ...
      if (file.size > 5 * 1024 * 1024)
        throw new Error("The optimized image must be smaller than 5 MB.");
      if (!file.type.startsWith("image/"))
        throw new Error("Only image files are supported.");
      const safe = file.name.replace(/[^a-zA-Z0-9._-]/g, "-");
      const storageRef = ref(
        firebaseStorage,
        `story-submissions/${session.id}/${crypto.randomUUID()}-${safe}`,
      );
    ...
    if (url === "/api/messages" && method === "GET") {
      await requireSession(["webmaster"]);
      const snapshot = await getDocs(collection(firestore, "contact_messages"));
      return {
        messages: sortNewest(
          snapshot.docs.map((item) =>
            withId<{ id: string; created_at: string } & Json>(
    ...
    if (url === "/api/settings" && method === "GET") {
      await requireSession(["webmaster"]);
      const snapshot = await getDoc(doc(firestore, "settings", "site"));
      return {
        settings: snapshot.data() ?? {},
        database: { provider: "Cloud Firestore", connected: true },
        storage: { provider: "Cloud Storage for Firebase" },
      } as T;
    }
    if (url === "/api/settings" && method === "PUT") {
      ...
      await setDoc(
        ...
        { ...(input.settings as Json), updated_at: now() },
        { merge: true },
      );
      return { ok: true } as T;
    }
    ...
    if (url === "/api/dashboard") {
      ...
      if (session.role === "member" || session.role === "website_user") {
        ...
        const verified = hours.docs
          ...
          .reduce((sum, item) => sum + Number(item.data().minutes ?? 0), 0);
        const pending = hours.docs
          .filter((item) => item.data().status === "pending")
          .reduce((sum, item) => sum + Number(item.data().minutes ?? 0), 0);
        return {
          user: session,
          totals: { verified_minutes: verified, pending_minutes: pending },
        } as T;
      }
      if (session.role === "volunteer_admin") {
        ...
        return {
          ...
          stats: {
            ...
            events: events.size,
            stories_pending: 0,
            hours_pending: hours.docs.filter(
      ...
      const [hours, events, users, stories] = await Promise.all([
        ...
        getDocs(collection(firestore, "events")),
        getDocs(collection(firestore, "users")),
        getDocs(collection(firestore, "stories")),
      ]);
      const verified = hours.docs
        .filter((item) => item.data().status === "verified")
        .reduce((sum, item) => sum + Number(item.data().minutes ?? 0), 0);
