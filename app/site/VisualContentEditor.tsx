...
function StructuredListInspector({kind,value,assets,onChange,onUpload}:{kind:"timeline"|"leaders"|"services";value:string;assets:MediaAsset[];onChange:(value:string)=>void;onUpload:(file:File,label:string)=>Promise<string>}){
  let items:Array<Record<string,string>>;try{items=JSON.parse(value) as Array<Record<string,string>>}catch{items=[]}
  const fields=kind==="timeline"?["title","label"]:kind==="leaders"?["name","role","image"]:["title","text","icon"];
  const labels:Record<string,string>={title:kind==="timeline"?"Year / stage":"Title",label:"Milestone",name:"Name",role:"Role",image:"Portrait",text:"Description",icon:"Music symbol"};
  function commit(next:Array<Record<string,string>>){onChange(JSON.stringify(next))}
  function update(index:number,key:string,nextValue:string){commit(items.map((item,itemIndex)=>itemIndex===index?{...item,[key]:nextValue}:item))}
  function add(){const blank:Record<string,string>=kind==="timeline"?{title:"NEW STAGE",label:"DESCRIBE THE MILESTONE"}:kind==="leaders"?{name:"New youth leader",role:"Role",image:""}:{title:"NEW SERVICE",text:"Describe this way to serve.",icon:"♪"};commit([...items,blank])}
  return <div className="visual-structured-list">{items.map((item,index)=><article key={index}><div className="visual-list-head"><b>{String(index+1).padStart(2,"0")}</b><button type="button" className="danger-link" onClick={()=>commit(items.filter((_,itemIndex)=>itemIndex!==index))}>Remove</button></div>{fields.map(field=>field==="image"?<div key={field}><label className="visual-field-label">{labels[field]}</label><ImagePicker value={item[field]??""} assets={assets} label={`${item.name||"Youth leader"} portrait`} onChange={next=>update(index,field,next)} onUpload={onUpload}/></div>:<label key={field}>{labels[field]}{field==="text"?<textarea value={item[field]??""} onChange={event=>update(index,field,event.target.value)}/>:<input value={item[field]??""} onChange={event=>update(index,field,event.target.value)}/>}</label>)}</article>)}<button className="outline-button visual-add-list" type="button" onClick={add}>+ Add {kind==="timeline"?"milestone":kind==="leaders"?"youth leader":"service card"}</button></div>
}
